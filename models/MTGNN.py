...
class MTGNN_c(nn.Module):
    def __init__(self, config, adj_mx):
        # self.month_embedding=nn.Embedding(13,2)
        super(MTGNN_c,self).__init__()
        self.adj_mx = adj_mx
        self.num_nodes = config.get('num_nodes', 1)
        self.feature_dim = config.get('feature_dim', 1)
        self.num_batches = config.get('num_batches', 1)
        ...
        self.end_channels = config.get('end_channels', 64)
...
class Model(nn.Module):
    def __init__(self, config, adj_mx):
        ...
        config=vars(config)
        self.adj_mx = adj_mx
        self.num_nodes = config.get('num_nodes', len(adj_mx))
        self.feature_dim = config.get('feature_dim_in', 2)
        self.num_batches = config.get('num_batches', 2)

        self._logger = getLogger()


        self.input_window = config.get('seq_len', 6)
        self.output_window = config.get('pred_len', 1)
    ...
    def get_pred_and_feature(self, x):
        ...
        x = F.relu(self.end_conv_1(x))
        x = self.end_conv_2(x)
        return x[..., :self.output_dim],feature.reshape(b,-1)
