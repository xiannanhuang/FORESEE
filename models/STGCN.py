...
class Model(nn.Module):
    def __init__(self, config, adj_mx):
        ...
        # self.blocks  =[[1 ,1024,2048], [2048, 1024, 1024]]
        self.input_window = config.get('input_window', 6)
        self.output_window = config.get('output_window', 1)
        self.drop_prob = config.get('dropout', 0.3)
    ...
    def forward(self, x):
        ...
        outputs=outputs.reshape(b,self.output_window,-1,n).transpose(-1,-2)
    ...
    def get_pred_and_feature(self, x):
        ...
        outputs = self.output(x_st2)  # (batch_size, output_dim(1), output_length(1), num_nodes)
        outputs = outputs.permute(0, 2, 3, 1)  # (batch_size, output_length(1), num_nodes, output_dim)
        return outputs,x_st1.reshape(b,-1)
    def get_loss(self,input,y,t,sigmai2,alpha):
        prediction=self.forward(input)
        ri = ((prediction - y)**2).reshape(y.shape[0], -1).sum(dim=1)
        lamba=t*(self.a)+self.b
        loss = (ri * (1 / sigmai2)).sum()+(-alpha*torch.log(lamba)+lamba/(sigmai2)).sum()+(self.a-1)**2+(self.b-1)**2
        return loss,ri
