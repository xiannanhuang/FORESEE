...
def calculate_normalized_laplacian(adj):
    ...
    Returns:
        np.ndarray: L
    """
    adj = sp.coo_matrix(adj)+sp.identity(adj.shape[0])
...
def calculate_random_walk_matrix(adj_mx):
    adj_mx = sp.coo_matrix(adj_mx)+sp.identity(adj_mx.shape[0])
...
class Seq2SeqAttrs:
    def __init__(self, config, adj_mx):
        ...
        self.input_dim = config.get('feature_dim', 2)
...
class DecoderModel(nn.Module, Seq2SeqAttrs):
    def __init__(self, config, adj_mx):
        nn.Module.__init__(self)
        Seq2SeqAttrs.__init__(self, config, adj_mx)
        self.output_dim = config.get('feature_dim_out', 1)
...
class Model(nn.Module):
    def __init__(self, config,adj_mx):
        data_feature=config
        self.data_feature=config
        self.adj_mx =adj_mx
        self.num_nodes = data_feature.get('num_nodes', 1)
        self.feature_dim = data_feature.get('feature_dim_in', 2)
        config['num_nodes'] = self.num_nodes
        config['feature_dim'] = self.feature_dim
        self.output_dim = data_feature.get('d_model', 2)
        # if config.get('model_confidence', False):
        #     self.output_dim = config.get('output_dim', 1)+2
    ...
    def forward(self, batch, batches_seen=None,contain_feature=False):
        """
        ...
        if contain_feature:
            return outputs, encoder_hidden_state.transpose(0, 1).reshape(batch_size,-1)
        return outputs
    def get_pred_and_feature(self,  batch):
        inputs = batch
        labels =None
        batch_size, _, num_nodes, input_dim = inputs.shape
        inputs = inputs.permute(1, 0, 2, 3)  # (input_window, batch_size, num_nodes, input_dim)
        ...
        # (num_layers, batch_size, self.hidden_state_size)
        self._logger.debug("Encoder complete")
        outputs = self.decoder(encoder_hidden_state, labels, batches_seen=None)
        # (self.output_window, batch_size, self.num_nodes * self.output_dim)
        self._logger.debug("Decoder complete")
        outputs = outputs.view(self.output_window, batch_size, self.num_nodes, self.output_dim).permute(1, 0, 2, 3)
        return outputs, encoder_hidden_state.transpose(0, 1).reshape(batch_size,-1)
