...
# Load project configuration from config.yaml
    ...
    # Configure logging
    # log_dir = config['log_dir']  # You can specify the directory for log files  239252
    ...
    # model1=model_dict[config['model']].Model(config,adj_mx).to(device)
    model=causal_model.CausalModel(model_dict[config['model']].Model,config,adj_mx).to(device)
    ...
    logging.info(f"Total valid_gird: {len(valid_gird)}")
    print(f"Total valid_gird: {len(valid_gird)}")
    for epoch in range(config['num_epochs']):
        ...
        print(f'Epoch [{epoch + 1}/{config["num_epochs"]}]: {val_loss:.4f}')
    ...
    model_path = os.path.join(config['model_dir'], f'{config["model"]}_{config["dataset_name"]}' + '_final_model.pth')
